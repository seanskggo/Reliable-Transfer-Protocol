...
class Receiver(TCP):
    ...
    def receive_opening(self) -> None:
        ...
        self.ack, rcv_seq, data, MSS, MWS, packet_type = self.decoder(struct.unpack("!II0sII2s", msg))
        self.log.append([Action.RECEIVE, self.get_time(), packet_type, self.ack, rcv_seq, len(data)])
        self.increment_ack(packet_type, data)
        self.MSS = MSS
        self.MWS = MWS
        self.addr = addr
    ...
    def receive(self) -> set:
        ...
        self.ack, rcv_seq, data, _, _, packet_type = self.decoder(struct.unpack(f"!II{self.MSS}sII2s", msg))
        self.log.append([Action.RECEIVE, self.get_time(), packet_type, self.ack, rcv_seq, len(data)])
        self.increment_ack(packet_type, data)
...
class Sender(TCP):
    ...
    def send_opening(self) -> None:
        ...
        self.log.append([Action.SEND, self.get_time(), Packet.SYN, self.seq, self.ack, len(Packet.NONE)])
